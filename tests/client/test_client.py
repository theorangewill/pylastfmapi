...
def test_get_search_data(mocker):
    ...
    mock_responses = [mock_response] * amount_responses
    mock_request_controller.request_search_pages.return_value = mock_responses

    client = LastFM('user_agent_test', 'api_key_test')
    ##
    response = client.get_search_data(payload, parent_key, list_key, amount)
    ##
    mock_request_controller.request_search_pages.assert_called_once()
    mock_request_controller.request_search_pages.assert_called_with(
        payload, parent_key, list_key, amount
    )
    assert response == [
        {'name': 'item1'},
...
def test_get_search_data_amount_not_divisible_by_limit(mocker):
    mocker.patch('pylastfm.client.LIMIT_SEARCH', 2)
    MockRequestController = mocker.patch(
        'pylastfm.client.RequestController', autospec=True
    )
    mock_request_controller = MockRequestController.return_value
